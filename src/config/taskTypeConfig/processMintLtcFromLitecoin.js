...
function convert(address) {
  try {
      ...
      // let message = "Please enter a valid address."
      console.log(err);
  }
  return '';
}

module.exports = class ProcessMintLtcFromLitecoin {
  ...
  //    "value": value,
  //    "taskType": "ProcessMintLtcFromBitcoin",
  //    "fee": fees.mintFeeBN
  //};
  async process(paramsJson) {
    ...
    try {
      ...
      //console.log("ProcessMintLtcFromBitcoin finishStep:", params.ccTaskId, paramsJson.stepIndex, p2sh.address);
      if (p2sh.address === "") {
  ...
  async generateOnetimeAddress(paramsJson, chainType, chainAddr, storemanGroupId, storemanGroupPublicKey) {
    ...
    try {
      ...
      let url = apiServerConfig.url + "/api/ltc/addAddrInfo";
      ...
      if (ret.data.success === true) {
        ...
        let checkLtcTxService = this.m_frameworkService.getService("CheckLtcTxService");
        data.fromBlockNumber = blockNumber;
        data.ccTaskId = params.ccTaskId;
        await checkLtcTxService.addOTAInfo(data);
        return {
          address: p2sh,
