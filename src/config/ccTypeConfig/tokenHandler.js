...
module.exports = class TokenHandler extends CCTypeHandleInterface { // ERC20 & ERC721
  ...
  async buildUserFastMint(steps, tokenPair, convert) {
    ...
    let networkFee = tool.parseFee(convert.fee, convert.value, unit, chainInfo.chainDecimals, {formatWithDecimals: false});
    let operateFee = tool.parseFee(convert.fee, convert.value, tokenPair.ancestorSymbol, decimals, {formatWithDecimals: false});
    let params = {
      ccTaskId: convert.ccTaskId,
      fromAddr: convert.fromAddr,
      scChainType: chainInfo.chainType,
  ...
  async checkGasFee(steps, tokenPair, convert) {
    ...
    if (chainInfo.chainType !== "TRX") {
      let unit = tool.getCoinSymbol(chainInfo.chainType, chainInfo.chainName);
      let fee = tool.parseFee(convert.fee, convert.value, unit, chainInfo.chainDecimals, {formatWithDecimals: false});
