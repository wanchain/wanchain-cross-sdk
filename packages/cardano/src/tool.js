...
function assetsToValue(assets) {
  ...
  policies.forEach((policy) => {
    let policyAssets = assets.filter((asset) => asset.unit.slice(0, 56) === policy);
    let assetsValue = wasm.Assets.new();
    policyAssets.forEach((asset) => {
      assetsValue.insert(wasm.AssetName.new(Buffer.from(asset.unit.slice(56), 'hex')), wasm.BigNum.from_str(asset.quantity));
    });
    multiAsset.insert(wasm.ScriptHash.from_bytes(Buffer.from(policy, 'hex')), assetsValue);
  });
  let value = wasm.Value.new(wasm.BigNum.from_str(lovelace ? lovelace.quantity : '0'));
...
function selectUtxos(utxos, rawOutput, protocolParameters) {
  let output = wasm.TransactionOutput.new(wasm.Address.from_bech32(rawOutput.address), assetsToValue(rawOutput.amount));
  let totalAssets = multiAssetCount(output.amount().multiasset());
  CoinSelection.setProtocolParameters(protocolParameters.coinsPerUtxoByte, protocolParameters.linearFee.minFeeA, protocolParameters.linearFee.minFeeB, protocolParameters.maxTxSize.toString());
  let outputs = wasm.TransactionOutputs.new();
  outputs.add(output); // adapt to CoinSelection api
  try {
    let selection = CoinSelection.randomImprove(utxos, outputs, 20 + totalAssets, rawOutput.address);
...
function genPlutusData() {
  let ls = wasm.PlutusList.new();
  ls.add(wasm.PlutusData.new_integer(wasm.BigInt.from_str('1')));
  return wasm.PlutusData.new_constr_plutus_data(wasm.ConstrPlutusData.new(wasm.BigNum.from_str('0'), ls));
}
...
async function checkUtxos(network, utxos, timeout = 0, interval = 5000) {
