...
export default (class ProcessAdaMintFromCardano {
  ...
  async process(stepData, wallet) {
    ...
    try {
      ...
      let txOutput = this.wasm.TransactionOutput.new(this.wasm.Address.from_bech32(params.crossScAddr), this.tool.assetsToValue(output.amount));
  ...
  buildTx(paymentAddr, inputs, output, networkFeeOutput, epochParameters, metaData) {
    ...
    const txBuilderConfig = wasm.TransactionBuilderConfigBuilder.new()
      ...
      .ex_unit_prices(wasm.ExUnitPrices.new(wasm.UnitInterval.new(wasm.BigNum.from_str("0"), wasm.BigNum.from_str("1")), wasm.UnitInterval.new(wasm.BigNum.from_str("0"), wasm.BigNum.from_str("1"))))
