...
class WanBridge extends EventEmitter {
  ...
  async init(iwanAuth, options = {}) {
    console.debug("SDK: init, network: %s, isTestMode: %s, smgName: %s, ver: 2404281832", this.network, this.isTestMode, this.smgName);
  ...
  async reclaim(taskId, wallet) {
    ...
    let errInfo = await this.txTaskHandleService.processTxTask({params}, wallet);
    if (!errInfo) {
